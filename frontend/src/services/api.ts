...
async function request<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  try {
    ...
    if (!r.ok) {
      ...
      try { const body = await r.json(); message = body.detail || body.message || message; } catch {}
...
export async function downloadReport(id: number, format: string) {
  ...
  if (!response.ok) {
    ...
    let message = `Request failed (${response.status})`; try { const body = await response.json(); message = body.detail || body.message || message; } catch {}
    throw new Error(message);
  }
