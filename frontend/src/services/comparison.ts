...
export function compareFindings(before: Finding[], after: Finding[]): FindingChange[] {
  const left = new Map(before.filter(f => f.finding_type !== 'risk_dimension' && f.finding_type !== 'provider_status').map(f => [keyOf(f), f]));
  const right = new Map(after.filter(f => f.finding_type !== 'risk_dimension' && f.finding_type !== 'provider_status').map(f => [keyOf(f), f]));
  const keys = Array.from(new Set([...left.keys(), ...right.keys()]));
  return keys.map(key => {
    const beforeFinding = left.get(key);
    const afterFinding = right.get(key);
    if (!beforeFinding) return { key, findingType: afterFinding!.finding_type, source: afterFinding!.source, value: afterFinding!.value, status: 'added', after: afterFinding };
    if (!afterFinding) return { key, findingType: beforeFinding.finding_type, source: beforeFinding.source, value: beforeFinding.value, status: 'removed', before: beforeFinding };
    ...
    return { key, findingType: afterFinding.finding_type, source: afterFinding.source, value: afterFinding.value, status: changed ? 'changed' : 'unchanged', before: beforeFinding, after: afterFinding };
  }).sort((a, b) => a.status.localeCompare(b.status) || a.findingType.localeCompare(b.findingType) || a.value.localeCompare(b.value));
}
