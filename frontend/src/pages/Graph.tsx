...
export function Graph({ id, findings = [], onEvidence, onPivot }: { id: number; findings?: Finding[]; onEvidence?: EvidenceCallback; onPivot?: (investigationId: number) => void }) {
  ...
  const visible = useMemo(() => {
    ...
    const edges = graph.edges.filter((edge) => edge.confidence >= filters.minConfidence && allowedTypes.has(edge.source) && allowedTypes.has(edge.target) && (filters.relation === 'all' || edge.relation === filters.relation));
  ...
  const neighborhoodIds = useMemo(() => (focusId ? new Set(neighborhood(visible, focusId, 1).nodeIds) : new Set<string>()), [visible, focusId]);
  const path = useMemo(() => (selectedId && focusId ? shortestPath(visible, selectedId, focusId) : []), [visible, selectedId, focusId]);
  ...
  function resetFilters() { setFilters(initialFilters); setSelectedId(''); setFocusId(''); }

  async function pivotEmail() {
    if (!selectedNode || selectedNode.type !== 'EMAIL' || !emailPattern.test(selectedNode.label.trim())) return;
    setPivoting(true); setError('');
    try { const created = await createInvestigation(selectedNode.label.trim(), false, true); onPivot?.(created.id); }
    catch (e) { setError(e instanceof Error ? e.message : 'Unable to create pivot investigation'); }
    finally { setPivoting(false); }
  }
  ...
  return (
    <div className="grid h-[650px] min-h-0 lg:grid-cols-[1fr_340px]">
      ...
      <aside className="overflow-y-auto border-t border-white/8 bg-[#0c0c0c] p-5 lg:border-l lg:border-t-0">
        ...
        <div className="mt-5 text-[11px] leading-4 text-zinc-600">Graph semantics: {graph.semantics || 'current_derived_view'}. Relationships retain their stored confidence; visual focus and filtering never create or merge evidence.</div>
      </aside>
    </div>
  );
}
