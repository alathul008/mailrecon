...
import {deleteInvestigation,downloadReport,getCorrelations,getGraph,getInvestigation,getTimeline,createInvestigation,type CorrelationResult,type GraphData} from '../services/api';
...
const tabs=['Overview','Correlations','Evidence','Providers','Timeline','Graph','Reports'] as const;
type Tab=typeof tabs[number];

export function Investigation({id,onBack,onOpen}:{id:number;onBack:()=>void;onOpen:(id:number)=>void}){
 const [inv,setInv]=useState<InvestigationType|null>(null);const [timeline,setTimeline]=useState<TimelineEvent[]>([]);const [graphData,setGraphData]=useState<GraphData>();const [correlations,setCorrelations]=useState<CorrelationResult>();const [tab,setTab]=useState<Tab>('Overview');const [loading,setLoading]=useState(true);const [error,setError]=useState('');const [deleting,setDeleting]=useState(false);const [pivoting,setPivoting]=useState(false);const [focusFindingId,setFocusFindingId]=useState<number>();
 const load=useCallback(async()=>{setLoading(true);setError('');try{const data=await getInvestigation(id);setInv(data);setTimeline(await getTimeline(id));setCorrelations(await getCorrelations(id))}catch(e){setError(e instanceof Error?e.message:'Unable to load investigation')}finally{setLoading(false)}},[id]);
 ...
 useEffect(()=>{if(!inv||!['queued','running'].includes(inv.status))return;const timer=window.setInterval(async()=>{try{const data=await getInvestigation(id);setInv(data);if(data.status!=='queued'&&data.status!=='running'){setTimeline(await getTimeline(id));setCorrelations(await getCorrelations(id))}}catch(e){setError(e instanceof Error?e.message:'Unable to refresh investigation')}},2000);return()=>window.clearInterval(timer)},[id,inv?.status]);
 ...
 return <div className="mx-auto max-w-[1500px] space-y-6 p-5 lg:p-8">
  ...
  {tab==='Reports'&&<div className="glass rounded-2xl p-6"><div className="flex items-center gap-3"><Download size={18} className="text-red-400"/><div><div className="font-medium">Evidence-preserving reports</div><div className="text-xs text-zinc-500">Use the existing report formats and provenance.</div></div></div><div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{['json','csv','html','pdf'].map(format=><button key={format} onClick={()=>report(format)} className="rounded-xl border border-white/10 bg-white/[.03] px-4 py-3 text-sm hover:bg-white/[.06]">Export {format.toUpperCase()}</button>)}</div><div className="mt-5 rounded-xl border border-white/7 bg-white/[.02] p-4 text-xs text-zinc-500">Reports preserve observed evidence separately from derived and correlated context.</div></div>}
