...
export function Investigation({id,onBack,onOpen}:{id:number;onBack:()=>void;onOpen:(id:number)=>void}){
 const [inv,setInv]=useState<InvestigationType|null>(null);const [timeline,setTimeline]=useState<TimelineEvent[]>([]);const [tab,setTab]=useState<Tab>('Overview');const [loading,setLoading]=useState(true);const [error,setError]=useState('');const [deleting,setDeleting]=useState(false);const [pivoting,setPivoting]=useState(false);const [focusFindingId,setFocusFindingId]=useState<number>();
 ...
 function openGraphEvidence(node:GraphNode){const finding=(inv?.findings||[]).find(f=>f.value===node.label||node.id.endsWith(`:${f.value}`));if(!finding){setError('No exact persisted finding is linked to this graph entity.');return}setFocusFindingId(finding.id);setTab('Evidence')}
 function pivotGraphNode(node:GraphNode){const finding=(inv?.findings||[]).find(f=>f.value===node.label||node.id.endsWith(`:${f.value}`));if(finding)void pivot(finding);else setError('This graph entity has no exact persisted email finding to pivot from.')}
 ...
 return <div className="mx-auto max-w-[1500px] space-y-6 p-5 lg:p-8">
  ...
  {tab==='Graph'&&<div className="glass overflow-hidden rounded-2xl"><div className="flex items-center gap-2 border-b border-white/8 px-5 py-4"><Network size={17} className="text-red-400"/><div><div className="font-medium">Evidence graph</div><div className="text-xs text-zinc-500">Relationships remain evidence-backed and probabilistic.</div></div></div><Graph id={inv.id} onEvidence={openGraphEvidence} onPivot={pivotGraphNode}/></div>}
