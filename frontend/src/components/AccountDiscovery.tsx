...
export function AccountDiscovery({ data, loading, error }: Props) {
  ...
  if (loading && !data) return <div className="glass rounded-2xl p-8 text-sm text-zinc-500">Discovering public accounts and profile associations…</div>;
  ...
  const accounts = data.services.filter(row => row.status === 'FOUND' || row.status === 'POSSIBLE');
