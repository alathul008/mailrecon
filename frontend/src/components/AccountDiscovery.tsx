...
export function AccountDiscovery({ data, loading, error }: Props) {
  ...
  return <div className="space-y-5">
    <div className="glass rounded-2xl p-5">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
          {[
            ...
            ['Profiles observed', summary.emailrep_profiles],
      ...
      <div className="mt-4 rounded-xl border border-white/7 bg-white/[.02] p-3 text-xs text-zinc-500">Account counts are evidence-driven. A FOUND result means a source reported an association; it does not by itself prove that the same human controls every profile.</div>
    </div>
    ...
    <section className="glass rounded-2xl p-5">
      ...
      {discovered.length === 0 ? <div className="mt-5 rounded-xl border border-white/7 bg-white/[.02] p-5 text-sm text-zinc-500">No account association was returned by the currently configured public sources.</div> : <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
        {discovered.map(row => <article key={`${row.provider}-${row.service}`} className="rounded-2xl border border-emerald-500/10 bg-white/[.02] p-4">
