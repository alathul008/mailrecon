...
export function ProviderStatusGrid({findings}:{findings:Finding[]}){
 return <div className="glass rounded-2xl p-5"><div className="font-medium">Provider execution</div><p className="mt-1 text-xs text-zinc-500">Operational state is distinct from intelligence. A provider failure is not a negative result; outages, rate limits, and no-result states are not evidence.</p><div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{findings.map(f=><div key={`${f.source}-${f.id}`} className="rounded-xl border border-white/7 bg-white/[.02] p-3"><div className="flex items-center justify-between gap-3"><div className="font-medium text-sm">{f.source}</div><Badge tone={tone(f.value)}>{states.includes(f.value)?label(f.value):'unknown state'}</Badge></div>{f.notes&&<div className="mt-2 text-xs text-zinc-500">{f.notes}</div>}</div>)}</div>{!findings.length&&<div className="mt-4 text-sm text-zinc-600">No provider status findings recorded. This does not mean providers failed.</div>}</div>
}
