import { ShieldAlert, Fingerprint, Server, Users, type LucideIcon } from 'lucide-react';
  ...
  return (
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
      {Object.entries(labels).map(([key, meta]) => {
        const value = Math.max(0, Math.min(100, Number(dimensions[key] || 0)));
        const I = meta.icon;

        return (
          <div key={key} className="rounded-xl border border-white/7 bg-white/[.025] p-4">
