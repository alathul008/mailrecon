...
def pdf_report(inv, findings, factors, provenance=None):
    ...
    story.append(Paragraph("Findings",styles["Heading2"])); rows=[["Source","Type","Value","Confidence","Severity","Attempt"]]
    ...
    for f in findings:
        ...
        rows.append([_pdf_text(f.source),_pdf_text(f.finding_type),_pdf_text(f.value)[:80],_pdf_text(f"{f.confidence:.0%}"),_pdf_text(f.severity),_pdf_text(attempt_label)])
    t=Table(rows,colWidths=[80,75,165,60,60,60],repeatRows=1); t.setStyle(TableStyle([("GRID",(0,0),(-1,-1),0.25,colors.grey),("BACKGROUND",(0,0),(-1,0),colors.lightgrey),("VALIGN",(0,0),(-1,-1),"TOP")])); story.append(t)
