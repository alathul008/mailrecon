...
def demo(db: Session=Depends(get_db)):
    ...
    db.add(inv); db.commit(); db.refresh(inv)
    demo_findings=[
      {"source":"Demo Dataset","source_url":"https://example.org/demo","finding_type":"email","value":"alex.morgan@fictional.test","confidence":1.0,"severity":"info","notes":"Completely fictional demo evidence."},
      {"source":"Demo Dataset","source_url":"https://example.org/demo","finding_type":"classification","value":"provider=Custom; type=Personal/Business unknown","confidence":0.95,"severity":"info","notes":"Fictional classification."},
      {"source":"Demo Dataset","source_url":"https://example.org/demo","finding_type":"profile_candidate","value":"https://github.com/alex-morgan-demo","confidence":0.72,"severity":"info","notes":"Possible match only; intentionally fictional."},
      {"source":"Demo Dataset","source_url":"https://example.org/demo","finding_type":"breach","value":"ExampleForum-2024 (fictional)","confidence":1.0,"severity":"high","notes":"Fictional breach record; no real leaked data."},
      {"source":"MailRecon Risk Engine","source_url":None,"finding_type":"risk_factor","value":"Known breach exposure (fictional demo)","confidence":1.0,"severity":"high","notes":"Score delta: +30"},
    ]
...
def timeline(inv_id:int,db:Session=Depends(get_db)):
    ...
    events=[]
    for f in fs:
        events.append({"timestamp":f.collected_at,"kind":"finding","label":f.finding_type,"source":f.source,"value":f.value,"severity":f.severity,"confidence":f.confidence})
    for m in mods:
...
def report(inv_id:int,format:str="json",db:Session=Depends(get_db)):
    ...
    if format=="csv":
        s=io.StringIO(); w=csv.writer(s); w.writerow(["source","type","value","confidence","severity","source_url","notes"]); [w.writerow([f.source,f.finding_type,f.value,f.confidence,f.severity,f.source_url,f.notes]) for f in fs]; return Response(s.getvalue(),media_type="text/csv",headers={"Content-Disposition":f'attachment; filename="mailrecon-{inv_id}.csv"'})
    if format=="html":
        body=f"<html><body><h1>MailRecon Report</h1><p>Target: {html.escape(inv.target)}</p><h2>Risk {inv.risk_score}/100 — {html.escape(str(inv.risk_level))}</h2><ul>"+"".join(f"<li><b>{html.escape(f.severity)}</b> {html.escape(f.finding_type)}: {html.escape(f.value)} ({f.confidence:.0%})</li>" for f in fs)+"</ul><p>OSINT findings are probabilistic and should be independently verified. Provider status is distinct from a negative finding.</p></body></html>"; return HTMLResponse(body)
    if format=="pdf":
