...
MODULE_TRANSITIONS = {
    "queued": {"queued", "running", "skipped", "abandoned"},
    "running": {"running", "completed", "failed", "skipped", "abandoned"},
    "completed": {"completed"},
    "failed": {"failed"},
    "skipped": {"skipped"},
    "abandoned": {"abandoned"},
...
def set_module(db,inv_id,name,status,message=None,token=None):
    if status not in MODULE_TRANSITIONS:
        raise ValueError(f"Unknown ModuleRun status: {status}")
    ...
    if not m:
        m=ModuleRun(investigation_id=inv_id,execution_id=execution_id,execution_attempt_id=execution_attempt_id,module=name)
        db.add(m)
        db.flush()
    if status not in MODULE_TRANSITIONS.get(m.status, set()):
        raise RuntimeError(f"Invalid ModuleRun transition: {m.status} -> {status}")
...
def add_findings(db,inv_id,fs,token=None):
    ...
    if not inv: return
    if token is None:
        execution_id=inv.execution_id; execution_attempt_id=inv.execution_attempt_id
...
def mark_investigation_failed(db, inv_id, token, exc):
    ...
    for m in db.scalars(select(ModuleRun).where(ModuleRun.investigation_id==inv_id,or_(ModuleRun.execution_attempt_id==execution_attempt_id,ModuleRun.execution_attempt_id.is_(None)))).all():
        ...
        if m.status == "running": set_module(db,inv_id,m.module,"failed",f"Investigation failed: {type(exc).__name__}",token=None)
        elif m.status == "queued": set_module(db,inv_id,m.module,"skipped","Not executed after investigation failure",token=None)
    finish_execution_attempt(db,inv_id,execution_attempt_id,"failed",reason=f"{type(exc).__name__}: investigation execution failed")
...
async def run_investigation(inv_id:int, token:str):
    ...
    with SessionLocal() as db:
        try:
            ...
            dns=await resolve(inv.domain); _require_ownership(db,inv_id,token)
            analysis["has_dmarc"]=record_presence(dns.get("DMARC",[]),dns.get("DMARC_status")); analysis["has_spf"]=record_presence(dns.get("SPF",[]),dns.get("SPF_status")); dnssec=dns.get("DNSSEC"); analysis["dnssec"]=dnssec if isinstance(dnssec,bool) else None
            fs=[]
            ...
            fs.append(finding("DNS","dnssec",dnssec_value,1.0 if analysis["dnssec"] is not None else 0.0,"info",notes="DNSSEC state is unknown when the resolver cannot validate it; unknown is not a security failure.")); add_findings(db,inv_id,fs,token)
            set_module(db,inv_id,"dns_analysis","completed","DNS analysis complete",token); set_module(db,inv_id,"domain_analysis","completed","Domain metadata derived from DNS/RDAP",token)
            for n in ["gravatar","rdap","public_profile_discovery","breach_sources"]: set_module(db,inv_id,n,"running",token=token)
            results=await run_providers(analysis["email"],analysis["domain"],candidates,allow_external=inv.external_provider_disclosure); _require_ownership(db,inv_id,token)
            ...
            for name,res in mapping:
                if isinstance(res,Exception): set_module(db,inv_id,name,"failed",f"Provider exception: {type(res).__name__}",token); add_findings(db,inv_id,[finding(name,"provider_status","error",1.0,"warning",notes=f"Provider raised {type(res).__name__}")],token)
                    ...
                    add_findings(db,inv_id,[provider_finding(res),*res.findings],token); module_status="completed" if res.status in {"ok","unconfigured","rate_limited","unavailable","disabled"} else "failed"; set_module(db,inv_id,name,module_status,res.message,token)
            ...
            risk=calculate({**analysis},[{"finding_type":r.finding_type,"confidence":r.confidence,"value":r.value,"notes":r.notes,"evidence_state":r.evidence_state,"raw_reference":r.raw_reference} for r in rows]); _capture_owned_execution(db,inv_id,token); inv.risk_score=risk.score; inv.risk_level=risk.level; db.commit()
            add_findings(db,inv_id,[finding("MailRecon Risk Engine","risk_factor",f["reason"],1.0,"high" if f["delta"]>10 else "medium" if f["delta"]>0 else "info",notes=f"Score delta: {f['delta']:+d}; dimension={f['dimension']}") for f in risk.factors],token); add_findings(db,inv_id,[finding("MailRecon Risk Engine","risk_dimension",f"{k}={v}",1.0,"info",notes="Dimension score, not a probability of compromise.") for k,v in risk.dimensions.items()],token); set_module(db,inv_id,"risk_calculation","completed",f"Risk score {risk.score}/100 ({risk.level})",token)
            ...
            _capture_owned_execution(db,inv_id,token); set_module(db,inv_id,"graph_build","completed","Relationship graph built with evidence-state-aware relationships",token); finish_execution_attempt(db,inv_id,execution_attempt_id,"completed")
            result=db.execute(update(Investigation).where(Investigation.id==inv_id,Investigation.status=="running",Investigation.execution_token==token).values(status="completed",completed_at=utcnow(),execution_heartbeat_at=None,execution_token=None)); db.commit()
