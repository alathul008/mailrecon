...
async def run_investigation(inv_id:int):
    with SessionLocal() as db:
        ...
        try:
            ...
            add_findings(db,inv_id,[finding("MailRecon","email",analysis["email"],1.0,"info",notes="Normalized and syntax-validated target."),finding("MailRecon","classification",f"provider={analysis['provider']}; type={'Disposable' if analysis['disposable'] else 'Role-based' if analysis['role_based'] else 'Personal/Business unknown'}",0.95,"info",notes="Classification, not an identity verdict.")])
            ...
            db.add_all([GraphEdge(investigation_id=inv_id,source=email_node.node_key,target=domain_node.node_key,relation="uses",confidence=1),GraphEdge(investigation_id=inv_id,source=email_node.node_key,target=user_node.node_key,relation="registered_as",confidence=.95)])
            rows=db.scalars(select(Finding).where(Finding.investigation_id==inv_id)).all(); seen=set()
            for r in rows:
                if r.finding_type in {"breach","profile_candidate","public_identity","profile"}:
                    ...
                    seen.add(key); db.add(GraphNode(investigation_id=inv_id,node_key=key,node_type=typ,label=r.value)); relation="appeared_in" if typ=="BREACH" else "possible_profile" if typ=="PROFILE" else "associated_identity"; db.add(GraphEdge(investigation_id=inv_id,source=email_node.node_key,target=key,relation=relation,confidence=r.confidence))
