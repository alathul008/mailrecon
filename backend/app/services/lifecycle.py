...
def finish_execution_attempt(db,inv_id,attempt_id,status,*,now=None,reason=None):
    ...
    now=now or utcnow(); result=db.execute(update(ExecutionAttempt).where(ExecutionAttempt.investigation_id==inv_id,ExecutionAttempt.execution_attempt_id==attempt_id,ExecutionAttempt.status=="running").values(status=status,finished_at=now,recovery_reason=reason).execution_options(synchronize_session=False)); return result.rowcount==1
def recover_stale_investigations(db,*,now=None):
    now=now or utcnow(); stale_before=_stale_before(now); stale=db.execute(select(Investigation.id,Investigation.execution_attempt_id).where(Investigation.status=="running",or_(Investigation.execution_heartbeat_at.is_(None),Investigation.execution_heartbeat_at<stale_before))).all(); recovered=0
    for inv_id,stale_attempt_id in stale:
        ...
        if stale_attempt_id: _mark_attempt_abandoned(db,inv_id,stale_attempt_id,now=now)
        recovered+=1
...
def claim_investigation(db,inv_id,*,now=None):
    now=now or utcnow(); token=secrets.token_hex(24); stale_before=_stale_before(now); generated_execution_id=str(uuid.uuid4()); generated_attempt_id=str(uuid.uuid4()); current=db.execute(select(Investigation.execution_id,Investigation.execution_attempt_id,Investigation.status,Investigation.execution_heartbeat_at).where(Investigation.id==inv_id)).one_or_none()
    if current is None: return None
    old_execution_id,old_attempt_id,old_status,old_heartbeat=current; stale_claim=old_status=="running" and (old_heartbeat is None or old_heartbeat<stale_before)
...
def fence_execution(db,inv_id,token):
    ...
    if result.rowcount!=1: db.rollback(); raise RuntimeError("Investigation execution lease is no longer owned")
...
def heartbeat_investigation(db,inv_id,token,*,now=None):
    now=now or utcnow(); result=db.execute(update(Investigation).where(Investigation.id==inv_id,Investigation.status=="running",Investigation.execution_token==token).values(execution_heartbeat_at=now).execution_options(synchronize_session=False)); db.commit(); return result.rowcount==1
