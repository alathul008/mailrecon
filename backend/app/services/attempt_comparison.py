...
def _projection(finding: Finding) -> dict:
    return {
        "id": finding.id, "source": finding.source, "source_url": finding.source_url,
        "finding_type": finding.finding_type, "value": finding.value, "confidence": finding.confidence,
        "severity": finding.severity, "evidence_state": finding.evidence_state,
        "execution_id": finding.execution_id, "execution_attempt_id": finding.execution_attempt_id,
        "first_seen": finding.first_seen, "last_seen": finding.last_seen,
        "collected_at": finding.collected_at, "notes": finding.notes,
    }
...
def _provider_states(findings: list[Finding]) -> dict[str, list[str]]:
    ...
    for finding in findings:
        if finding.finding_type == "provider_status": states[finding.source].append(finding.value)
...
def _risk_dimensions(findings: list[Finding]) -> dict[str, int]:
    result: dict[str, int] = {}
    for finding in findings:
        if finding.finding_type != "risk_dimension" or "=" not in finding.value: continue
...
def _risk_factor_deltas(findings: list[Finding]) -> list[dict]:
    result = []
    for finding in findings:
        if finding.finding_type != "risk_factor": continue
        delta = None
        if finding.notes and "Score delta:" in finding.notes:
            try: delta = int(finding.notes.split("Score delta:", 1)[1].split(";", 1)[0].strip())
...
def compare_attempts(db: Session, investigation_id: int, before_attempt_id: str, after_attempt_id: str) -> dict:
    if before_attempt_id == after_attempt_id: raise ValueError("Comparison requires two distinct execution attempts")
    attempts = db.scalars(select(ExecutionAttempt).where(ExecutionAttempt.investigation_id == investigation_id, ExecutionAttempt.execution_attempt_id.in_([before_attempt_id, after_attempt_id]))).all()
    by_id = {attempt.execution_attempt_id: attempt for attempt in attempts}
    before, after = by_id.get(before_attempt_id), by_id.get(after_attempt_id)
    if before is None or after is None: raise LookupError("Execution attempt not found for investigation")

    findings = db.scalars(select(Finding).where(Finding.investigation_id == investigation_id, Finding.execution_attempt_id.in_([before_attempt_id, after_attempt_id]), Finding.finding_type.not_in(EXCLUDED_FINDING_TYPES)).order_by(Finding.source.asc(), Finding.finding_type.asc(), Finding.value.asc(), Finding.id.asc())).all()
    ...
    left, right = {_finding_key(f): f for f in before_findings}, {_finding_key(f): f for f in after_findings}

    provider_findings = db.scalars(select(Finding).where(Finding.investigation_id == investigation_id, Finding.execution_attempt_id.in_([before_attempt_id, after_attempt_id]), Finding.finding_type == "provider_status").order_by(Finding.source.asc(), Finding.value.asc(), Finding.id.asc())).all()
    ...
    added, removed, changed, unchanged, inconclusive = [], [], [], [], []
    for key in sorted(set(left) | set(right)):
        ...
        if old is None and new is not None:
            ...
            old_view, new_view = _projection(old), _projection(new)
            comparable = ("confidence", "severity", "evidence_state", "source_url", "notes")
            entry = {"key": list(key), "before": old_view, "after": new_view}
            (changed if any(old_view[field] != new_view[field] for field in comparable) else unchanged).append(entry)

    provider_sources = sorted(set(before_provider) | set(after_provider))
    operational = [{"provider": source, "before": before_provider.get(source, []), "after": after_provider.get(source, [])} for source in provider_sources if before_provider.get(source, []) != after_provider.get(source, [])]
    ...
    risk_rows = db.scalars(select(Finding).where(Finding.investigation_id == investigation_id, Finding.execution_attempt_id.in_([before_attempt_id, after_attempt_id]), Finding.finding_type == "risk_dimension").order_by(Finding.value.asc(), Finding.id.asc())).all()
    risk_before = _risk_dimensions([f for f in risk_rows if f.execution_attempt_id == before_attempt_id])
    risk_after = _risk_dimensions([f for f in risk_rows if f.execution_attempt_id == after_attempt_id])
    ...
    modules = db.scalars(select(ModuleRun).where(ModuleRun.investigation_id == investigation_id, ModuleRun.execution_attempt_id.in_([before_attempt_id, after_attempt_id])).order_by(ModuleRun.module.asc(), ModuleRun.execution_attempt_id.asc())).all()
    module_projection = [{"module": m.module, "status": m.status, "message": m.message, "execution_attempt_id": m.execution_attempt_id, "started_at": m.started_at, "finished_at": m.finished_at} for m in modules]

    return {
        "investigation_id": investigation_id, "semantics": "attempt_comparison", "identity_semantics": "source + finding_type + value",
