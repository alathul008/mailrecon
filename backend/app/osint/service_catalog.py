...
_IMPLEMENTED_SERVICES: tuple[ServiceDefinition, ...] = tuple(ServiceDefinition(item.name, item.category, item.discovery_methods, item.supported, item.name) for item in provider_definitions(account_discovery=True))
...
_UNSUPPORTED_SERVICES: tuple[ServiceDefinition, ...] = (
    ...
    ServiceDefinition("Twitch", "Streaming", ("public_profile_page",), False),
...
def _provider_status(findings: list[dict[str, Any]], provider: str | None) -> str | None:
    ...
    for item in findings:
        if item.get("source") == provider and item.get("finding_type") == "provider_status": return str(item.get("value"))
    return None
...
def _service_from_url(source_url: Any) -> str | None:
    ...
    return {"github.com":"GitHub","gitlab.com":"GitLab","reddit.com":"Reddit","dev.to":"Dev.to","codeberg.org":"Codeberg","keybase.io":"Keybase","huggingface.co":"Hugging Face","bitbucket.org":"Bitbucket","medium.com":"Medium","vimeo.com":"Vimeo","flickr.com":"Flickr","soundcloud.com":"SoundCloud","patreon.com":"Patreon","buymeacoffee.com":"Buy Me a Coffee","linktr.ee":"Linktree","about.me":"About.me","behance.net":"Behance","dribbble.com":"Dribbble","producthunt.com":"Product Hunt","kaggle.com":"Kaggle","npmjs.com":"npm","pypi.org":"PyPI","hub.docker.com":"Docker Hub","codepen.io":"CodePen","replit.com":"Replit","instructables.com":"Instructables","wattpad.com":"Wattpad","goodreads.com":"Goodreads","pinterest.com":"Pinterest","tumblr.com":"Tumblr","x.com":"X","instagram.com":"Instagram","threads.net":"Threads","youtube.com":"YouTube"}.get(host)
...
def _emailrep_rows(findings: list[dict[str, Any]]) -> list[dict[str, Any]]:
    rows=[]; seen=set(); provider_status=_provider_status(findings,"Email Intelligence")
    for f in findings:
...
def build_account_discovery_matrix(findings: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    matrix.extend(_emailrep_rows(findings)); return matrix
