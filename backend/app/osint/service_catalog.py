...
def _public_network_rows(findings: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    seen: set[str] = set()
    for f in findings:
        if f.get("source") != "Public Profile Network" or f.get("finding_type") != "profile_candidate": continue
        source_url = str(f.get("source_url") or ""); host = urlparse(source_url).netloc.lower().removeprefix("www."); service_meta = _PUBLIC_NETWORK_HOSTS.get(host)
        if not service_meta: continue
        service, category = service_meta
        if service in seen: continue
        seen.add(service)
        rows.append({"category":category,"service":service,"status":"POSSIBLE","supported":True,"provider":"Public Profile Network","discovery_methods":["passive username profile lookup"],"identifier":f.get("value"),"confidence":float(f.get("confidence")) if f.get("confidence") is not None else None,"provider_status":_provider_status(findings,"Email Intelligence") or "ok","checked_at":f.get("collected_at"),"evidence":[{"finding_id":f.get("id"),"finding_type":f.get("finding_type"),"evidence_state":f.get("evidence_state"),"confidence":f.get("confidence"),"source":f.get("source"),"source_url":source_url,"notes":f.get("notes")}]})
    return rows
...
def build_account_discovery_matrix(findings: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    matrix.extend(_emailrep_rows(findings)); matrix.extend(_public_network_rows(findings)); return matrix
