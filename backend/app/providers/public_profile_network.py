...
class PublicProfileNetworkProvider:
    ...
    async def _json_probe(self, service: str, url: str, *, params: dict[str, str] | None = None) -> tuple[str, dict | None, str | None]:
        ...
        async with httpx.AsyncClient(timeout=settings.request_timeout_seconds, headers={"accept": "application/json", "user-agent": "MailRecon/1.2 passive profile discovery"}, follow_redirects=False, trust_env=False, transport=pinned_transport(url)) as client:
    ...
    async def _page_probe(self, service: str, url: str) -> tuple[str, dict | None, str | None]:
        settings = get_settings()
        validate_provider_url(url)
        async with httpx.AsyncClient(timeout=settings.request_timeout_seconds, headers={"accept": "text/html,application/xhtml+xml", "user-agent": "MailRecon/1.2 passive profile discovery"}, follow_redirects=False, trust_env=False, transport=pinned_transport(url)) as client:
        ...
        if failure:
            return failure.status, None, failure.message
        return ("found", {"http_status": response.status_code}, None) if 200 <= response.status_code < 300 else ("unknown", None, f"{service}: unexpected HTTP {response.status_code}")
    ...
    async def _probe(self, service: str, username: str) -> tuple[str, str, str, dict | None, str | None]:
        ...
        try:
            if service == "GitHub":
                status, data, message = await self._page_probe(service, f"https://github.com/{encoded}")
    ...
    async def run(self, context: ProviderContext) -> ProviderResult:
        ...
        services = ("GitHub", "Reddit", "Dev.to", "Codeberg", "Keybase", "Hugging Face")
        ...
        for service, status, profile_url, data, message in results:
            if status == "found":
                ...
                findings.append(finding(self.name, "profile_candidate", username, 0.72, "info", profile_url, notes=f"Evidence state: {EVIDENCE_OBSERVED}. Public {service} profile was observed for the derived username. The profile existence is observed; the relationship between this username and the target email remains a possible correlation.", raw_reference={"service": service, "username": username, "profile": details, "evidence_state": EVIDENCE_OBSERVED}))
            elif status in {"error", "rate_limited", "unavailable"}:
                unavailable += 1
                findings.append(finding(self.name, "service_status", f"{service}:{status}", 1.0, "warning", profile_url or None, notes=message or f"{service} could not be checked."))
            else:
                findings.append(finding(self.name, "service_status", f"{service}:no_public_evidence", 1.0, "info", profile_url or None, notes=f"No public {service} profile was observed for the derived username. This is not proof that no account exists."))
        status = "ok" if found or unavailable < len(services) else "unavailable"
        return ProviderResult(self.name, status, findings=findings, message=f"{found} public username profiles observed across {len(services)} services" + (f"; {unavailable} services unavailable" if unavailable else ""))
