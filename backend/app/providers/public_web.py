...
class PublicWebProvider:
    ...
    def _queries(self, email: str, candidates: list[str]) -> list[str]:
        """Legacy general query plan kept stable for deterministic callers."""
        queries = [f'"{email}"']
    ...
    async def run(self, email: str, candidates: list[str]) -> ProviderResult:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=settings.request_timeout_seconds, headers=headers, follow_redirects=False, trust_env=False, transport=pinned_transport(endpoint)) as client:
                for query, requested_service in service_query_plan(email, candidates):
                    ...
                    for item in self._results(data)[:10]:
                        ...
                        findings.append(finding(
                            ...
                            "info",
                            result_url,
                            notes="Public search result only; this does not confirm an account or human identity. Evidence state: possible_match.",
                            raw_reference={"query": query, "url": result_url, "title": title, "source": item.get("engine") or item.get("source"), "service": service, "category": matched_service.category if matched_service else None, "evidence_state": EVIDENCE_POSSIBLE},
                            evidence_state=EVIDENCE_POSSIBLE,
                        ))
            deduped, seen = [], set()
            for item in findings:
                key = (item.get("source_url"), item.get("value"), item.get("raw_reference", {}).get("service"))
                if key in seen:
                    continue
                seen.add(key)
                deduped.append(item)
            return ProviderResult(self.name, "ok", findings=deduped, message=f"Collected {len(deduped)} public search references without storing page contents.")
