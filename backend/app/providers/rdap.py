...
class RDAPProvider:
    ...
    async def run(self, domain: str) -> ProviderResult:
        settings = get_settings(); url = f"https://rdap.org/domain/{domain}"
        try:
            ...
            if response.status_code == 404: return ProviderResult(self.name, "ok", message="No public RDAP registration returned")
            ...
            findings=[]; ldh_name=data.get("ldhName")
