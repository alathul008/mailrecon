...
async def test_multiple_provider_tasks_are_cancelled_and_drained(monkeypatch):
    ...
    class Provider:
        ...
        async def run(self,*args):
            ...
            except asyncio.CancelledError: cancelled[self.name].set(); raise
    install_fake_providers(monkeypatch,Provider)
    real_create_task=orchestrator.asyncio.create_task
    def capture_task(coro):
        task=real_create_task(coro)
        if getattr(coro,"cr_code",None) and coro.cr_code.co_name=="_run_provider_call": provider_tasks.append(task)
        return task
    monkeypatch.setattr(orchestrator.asyncio,"create_task",capture_task)
    ownership=iter([True,False]); monkeypatch.setattr(orchestrator,"execution_is_owned",lambda db,inv_id,token:next(ownership))
    with pytest.raises(orchestrator.ProviderOwnershipLost): await orchestrator.run_providers("test@example.com","example.com",["test"],inv_id=1,token="token")
    assert all(event.is_set() for event in started.values()); assert all(event.is_set() for event in cancelled.values()); assert len(provider_tasks)==len(names); assert all(task.done() for task in provider_tasks)
...
async def test_provider_completion_before_final_ownership_check_is_rejected(monkeypatch):
    ...
    async def drive_race(): await provider_done.wait(); owned["value"]=False; release_monitor.set()
...
async def test_provider_completion_after_ownership_loss_is_rejected(monkeypatch):
    ...
    def ownership_probe(db,inv_id,token):
        ...
        if first_check: first_check=False; ownership_checked.set(); return True
        ownership_lost.set(); return owned["value"]
    monkeypatch.setattr(orchestrator,"execution_is_owned",ownership_probe)
    async def drive_race(): await started.wait(); await ownership_checked.wait(); owned["value"]=False; release.set()
    driver=asyncio.create_task(drive_race())
    try:
        with pytest.raises(orchestrator.ProviderOwnershipLost): await orchestrator.run_providers("test@example.com","example.com",["test"],inv_id=1,token="token")
    finally: await driver
...
async def test_worker_a_is_fenced_after_worker_b_recovery(monkeypatch,tmp_path):
    engine=make_engine(tmp_path); SessionLocal=sessionmaker(engine,expire_on_commit=False,class_=Session); monkeypatch.setattr(orchestrator,"SessionLocal",SessionLocal)
    ...
    release.set();
    with pytest.raises(orchestrator.ProviderOwnershipLost): await worker_a
    with SessionLocal() as db:
        current=db.get(Investigation,inv_id); assert current.execution_token==token_b; assert current.status=="running"
        with pytest.raises(RuntimeError,match="lease is no longer owned"): orchestrator.add_findings(db,inv_id,[{"source":"Gravatar","finding_type":"profile","value":"stale-a","confidence":1,"severity":"info"}],token=token_a)
        assert db.scalars(select(Finding).where(Finding.investigation_id==inv_id,Finding.value=="stale-a")).all()==[]; attempts=db.scalars(select(ExecutionAttempt).where(ExecutionAttempt.investigation_id==inv_id)).all(); assert len(attempts)==2; assert {attempt.status for attempt in attempts}=={"abandoned","running"}; assert any(attempt.execution_attempt_id==current.execution_attempt_id and attempt.status=="running" for attempt in attempts)
...
async def test_deletion_during_active_provider_work_cannot_resurrect_data(monkeypatch,tmp_path):
    ...
    release.set()
    with pytest.raises(orchestrator.ProviderOwnershipLost): await worker
    with SessionLocal() as db:
        assert db.get(Investigation,inv_id) is None; assert db.scalars(select(ExecutionAttempt).where(ExecutionAttempt.investigation_id==inv_id)).all()==[]; assert db.scalars(select(ModuleRun).where(ModuleRun.investigation_id==inv_id)).all()==[]; assert db.scalars(select(Finding).where(Finding.investigation_id==inv_id)).all()==[]
...
async def test_cancellation_exception_cannot_turn_ownership_loss_into_success(monkeypatch):
    ...
    class Provider:
        async def run(self,*args):
            started.set()
            try: await asyncio.Event().wait()
            except asyncio.CancelledError: cleanup_failed.set(); raise RuntimeError("unexpected cancellation cleanup failure")
    install_fake_providers(monkeypatch,Provider); ownership=iter([True,False]); monkeypatch.setattr(orchestrator,"execution_is_owned",lambda db,inv_id,token:next(ownership))
    with pytest.raises(orchestrator.ProviderOwnershipLost): await orchestrator.run_providers("test@example.com","example.com",["test"],inv_id=1,token="token")
    assert started.is_set(); assert cleanup_failed.is_set()
