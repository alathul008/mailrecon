...
def test_csv_safe_neutralizes_formula_prefixes():
    for value in ["=1+1","+1+1","-1+1","@SUM(A1)","\t=1+1","\n+1+1"]: assert csv_safe(value).startswith("'")
    assert csv_safe("ordinary evidence")=="ordinary evidence"; assert csv_safe(None) is None

@pytest.mark.asyncio
async def test_investigation_queue_depth_is_enforced(tmp_path):
    engine=create_engine(f"sqlite:///{tmp_path/'queue.db'}"); Base.metadata.create_all(engine); settings=get_settings(); old_limit=settings.max_queue_depth; settings.max_queue_depth=1
    try:
        with Session(engine) as db:
            first=await create(InvestigationCreate(email="one@example.com"),db); assert first["status"]=="queued"
            with pytest.raises(HTTPException) as exc: await create(InvestigationCreate(email="two@example.com"),db)
            assert exc.value.status_code==429
    finally: settings.max_queue_depth=old_limit
...
def test_runtime_schema_bootstrap_reaches_head_and_detects_physical_drift(tmp_path,monkeypatch):
    ...
    try:
        ...
        with engine.connect() as conn: assert conn.execute(text("SELECT version_num FROM alembic_version")).scalar_one()=="0007"
...
def test_runtime_schema_rejects_missing_execution_integrity_index(tmp_path,monkeypatch):
    engine=create_engine(f"sqlite:///{tmp_path/'index-drift.db'}"); monkeypatch.setattr(schema,"engine",engine); settings=get_settings(); old_url=settings.database_url; settings.database_url=f"sqlite:///{tmp_path/'index-drift.db'}"
    try:
        schema.ensure_schema()
        with engine.begin() as conn: conn.execute(text("DROP INDEX uq_module_runs_attempt_module"))
        assert "execution_attempt_id" in {column["name"] for column in inspect(engine).get_columns("module_runs")}
        with pytest.raises(RuntimeError,match="execution indexes diverge"): schema.ensure_schema()
    finally: settings.database_url=old_url

def test_runtime_schema_bootstraps_unversioned_phase6_database(tmp_path,monkeypatch):
    db_path=tmp_path/"legacy.db"; cfg=migration_config(db_path); command.upgrade(cfg,"0002"); engine=create_engine(f"sqlite:///{db_path}")
    with engine.begin() as conn: conn.execute(text("DROP TABLE alembic_version"))
