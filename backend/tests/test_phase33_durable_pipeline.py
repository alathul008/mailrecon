...
async def test_durable_pipeline_persists_current_attempt_and_projects_public_web(monkeypatch):
    ...
    try:
        ...
        with TestSessionLocal() as db:
            ...
            first_attempt=first.execution_attempt_id; first_execution=first.execution_id; public=db.scalars(select(Finding).where(Finding.investigation_id==inv_id,Finding.source=="Public Web")).all(); assert public; assert all(row.evidence_state=="possible_match" for row in public); assert all(row.execution_attempt_id==first_attempt for row in public)
            ...
            discovery=account_discovery(inv_id,db); discovery_web=next(row for row in discovery["services"] if row["service"]=="Public Web"); assert discovery_web["status"]=="POSSIBLE"; assert discovery_web["evidence"][0]["execution_attempt_id"]==first_attempt
