...
async def test_hibp_without_key_is_unconfigured(monkeypatch):
    ...
    try: result = await HIBPProvider().run("user@example.com")
...
async def test_hibp_404_is_legitimate_no_result(monkeypatch):
    ...
    try:
        patch_client(monkeypatch, [hibp_module], status=404, json_data={}); result = await HIBPProvider().run("user@example.com")
    finally: settings.hibp_api_key = original
...
async def test_hibp_http_failures(monkeypatch, status, expected):
    settings = hibp_module.get_settings(); original = settings.hibp_api_key; settings.hibp_api_key = "test-key"
    try:
        patch_client(monkeypatch, [hibp_module], status=status, json_data={}); result = await HIBPProvider().run("user@example.com")
    finally: settings.hibp_api_key = original
...
async def test_hibp_network_failures_are_unavailable(monkeypatch, exception):
    settings = hibp_module.get_settings(); original = settings.hibp_api_key; settings.hibp_api_key = "test-key"
    try:
        patch_client(monkeypatch, [hibp_module], exception=exception); result = await HIBPProvider().run("user@example.com")
    finally: settings.hibp_api_key = original
...
async def test_hibp_malformed_json_has_no_positive_findings(monkeypatch):
    settings = hibp_module.get_settings(); original = settings.hibp_api_key; settings.hibp_api_key = "test-key"
    try:
        patch_client(monkeypatch, [hibp_module], content=b"not-json"); result = await HIBPProvider().run("user@example.com")
    finally: settings.hibp_api_key = original
...
async def test_hibp_incomplete_and_malformed_records_are_not_findings(monkeypatch):
    settings = hibp_module.get_settings(); original = settings.hibp_api_key; settings.hibp_api_key = "test-key"
    try:
        patch_client(monkeypatch, [hibp_module], json_data=[{}, "bad", {"Name": "Valid", "BreachDate": "not-a-date"}]); result = await HIBPProvider().run("user@example.com")
    finally: settings.hibp_api_key = original
...
async def test_github_http_failures(monkeypatch, status, expected):
    patch_client(monkeypatch, [github_module], status=status, json_data={}); result = await GitHubProvider().run(["example"], "user@example.com")
    assert result.status == expected; assert result.findings == []

@pytest.mark.asyncio
async def test_github_timeout_and_malformed_json(monkeypatch):
    patch_client(monkeypatch, [github_module], exception=httpx.ReadTimeout("timed out")); result = await GitHubProvider().run(["example"], "user@example.com")
    assert result.status == "unavailable"; assert result.findings == []
    patch_client(monkeypatch, [github_module], content=b"not-json"); result = await GitHubProvider().run(["example"], "user@example.com")
    assert result.status == "error"; assert result.findings == []
...
async def test_provider_timeout_uses_configured_value(monkeypatch):
    ...
    try:
        settings.hibp_api_key = "test-key"; patch_client(monkeypatch, [hibp_module], status=404, json_data={}); result = await HIBPProvider().run("user@example.com")
    finally:
...
async def test_provider_execution_isolated_when_one_raises(monkeypatch):
    async def ok_gravatar(self, email): return ProviderResult("Gravatar", "ok", message="done")
    async def broken_rdap(self, domain): raise RuntimeError("boom")
    async def ok_github(self, candidates, email): return ProviderResult("GitHub", "ok", message="done")
    async def ok_gitlab(self, email): return ProviderResult("GitLab", "ok", message="done")
    async def unconfigured_hibp(self, email): return ProviderResult("Have I Been Pwned", "unconfigured")
...
async def test_rdap_ssrf_validation_failure_is_provider_error(monkeypatch):
    ...
    result = await RDAPProvider().run("example.com"); assert result.status == "error"; assert result.findings == []
