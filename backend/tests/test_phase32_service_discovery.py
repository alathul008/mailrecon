...
def test_service_query_plan_is_bounded_and_service_scoped():
    ...
    assert ('"target" site:steamcommunity.com', "Steam") in plan
    assert ('"target@example.com" site:steamcommunity.com', "Steam") not in plan
    assert len(plan) == 21
...
def test_public_web_evidence_populates_game_service_without_identity_claim():
    findings = [{
        ...
        "value": "Example Steam profile",
        "confidence": 0.55,
        "evidence_state": "possible_match",
        "source_url": "https://steamcommunity.com/id/example",
        "service": "Steam",
    }]
    ...
    assert steam["evidence"][0]["evidence_state"] == "possible_match"
