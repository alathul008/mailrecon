...
async def test_public_web_is_explicitly_unconfigured_without_endpoint(monkeypatch):
    ...
    result = await PublicWebProvider().run("target@example.com", ["target", "target_dev"])
    assert result.status == "unconfigured"
    assert result.findings == []


def test_public_web_query_plan_is_bounded_and_exact_email_first():
    queries = PublicWebProvider()._queries(
        "target@example.com",
        ["target", "target_dev", "third", "fourth", "fifth"],
    )
    assert queries == [
        '"target@example.com"',
        '"target"',
    ...
    findings = [
        {
            ...
            "value": "Example public result",
            "confidence": 0.55,
            "evidence_state": "possible_match",
            "source_url": "https://example.test/public-result",
        }
    ]
