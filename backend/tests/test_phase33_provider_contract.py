...
def test_registry_contract_invariants_and_catalogue_consistency():
    ...
    assert {item.name for item in executable if item.factory} == {
        "Gravatar", "RDAP", "GitHub", "GitLab", "Have I Been Pwned", "Public Web"
    }
    account_discovery = registry.provider_definitions(account_discovery=True)
    assert {item.name for item in account_discovery} == {
...
@pytest.mark.parametrize(
    ...
    [
        (GravatarProvider, "Gravatar"),
        (RDAPProvider, "RDAP"),
        (GitHubProvider, "GitHub"),
        (GitLabProvider, "GitLab"),
        (HIBPProvider, "Have I Been Pwned"),
        (PublicWebProvider, "Public Web"),
    ],
)
async def test_each_executable_provider_accepts_canonical_context_through_registry(monkeypatch, provider_class, provider_name):
    ...
    async def run(self, context):
        seen.append(context)
        return ProviderResult(provider_name, "ok")

    monkeypatch.setattr(provider_class, "run", run)
    definition = registry.provider_definition(provider_name)
    result = await registry.execute(definition, context=CONTEXT)
...
async def test_public_web_registry_execution_uses_email_and_candidates_correctly(monkeypatch):
    ...
    monkeypatch.setattr(public_web_module.httpx, "AsyncClient", FakeAsyncClient)
    ...
    assert all(f["evidence_state"] == "possible_match" for f in result.findings)
    assert all(f["confidence"] == 0.55 for f in result.findings)
    assert any('"user@example.com"' in url for url in FakeAsyncClient.queries)
    assert any('"user"' in url for url in FakeAsyncClient.queries)
    assert any('"example"' in url for url in FakeAsyncClient.queries)
