    ...
    async def run(self, context):
        if self.name == "Gravatar":
            ...
            findings = [finding(
                ...
                raw_reference={"login": "alice", "evidence_state": EVIDENCE_SOURCE_ASSOCIATED},
            )]
            ...
            findings = [finding(
                ...
                raw_reference={"data_classes": ["Email addresses"]},
            ...
            findings = [finding(
                ...
                raw_reference={"evidence_state": "possible_match"},
...
def test_authoritative_full_pipeline_is_durable_and_provider_network_free(monkeypatch, tmp_path):
    ...
    async def fake_dns(_domain):
        return {
            "A": ["93.184.216.34"], "AAAA": [], "MX": ["10 mail.example.com"],
            "NS": [], "CNAME": [], "SPF_status": "ok", "SPF_analysis": {"policy": "-all"},
            "DMARC_status": "ok", "DMARC_analysis": {"policy": "quarantine"},
            "DKIM_analysis": [], "DNSSEC": True, "MX_PROVIDER": "Example Mail",
            "NS_PROVIDER": None, "IP_CONTEXT": [],
        }
    ...
    monkeypatch.setattr(orchestrator.OllamaProvider, "summarize", fake_summary)
    ...
    try:
        with TestClient(app) as client:
            ...
            with Session() as db:
                ...
                assert inv.execution_id and inv.execution_attempt_id
                ...
                assert attempt.execution_id == inv.execution_id and attempt.status == "completed"
                ...
                assert modules
                assert all(m.execution_id == inv.execution_id and m.execution_attempt_id == inv.execution_attempt_id for m in modules)
            ...
            assert any(e["label"] == "breach" for e in timeline_data)
            ...
            assert "raw_reference" not in report_data["findings"][0]
