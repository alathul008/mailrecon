...
def test_graph_migration_removes_only_exact_duplicates(tmp_path):
    db_path = tmp_path / "graph-duplicates.db"
...
def test_graph_migration_refuses_ambiguous_duplicates(tmp_path):
    db_path = tmp_path / "graph-ambiguous.db"
    cfg = migration_config(db_path)
    command.upgrade(cfg, "0005")
    engine = create_engine(f"sqlite:///{db_path}")
    with engine.begin() as conn:
...
def test_graph_view_is_sorted_and_declares_current_derived_provenance(tmp_path):
    ...
    Base.metadata.create_all(engine)
    with Session(engine) as db:
        inv = Investigation(target="test@example.com", normalized_email="test@example.com", username="test", domain="example.com", status="completed", execution_id="exec-1")
        db.add(inv)
        db.commit()
        db.refresh(inv)
        ...
        db.flush()
        inv.execution_attempt_id = "attempt-1"
        db.add_all([
...
def test_html_report_labels_unspecified_finding_without_attempt_as_unspecified(tmp_path):
    ...
    with Session(engine) as db:
        ...
        db.commit()
        db.refresh(inv)
        attempt = ExecutionAttempt(investigation_id=inv.id, execution_id="exec-1", execution_attempt_id="attempt-1", status="completed")
        db.add(attempt)
        db.flush()
        inv.execution_attempt_id = "attempt-1"
